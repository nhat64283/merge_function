...
int send_file(int conn_sock, char* dir_name) { //char dir_name[] = "Downloadable files";
	...
	if(file_path == NULL) // error occurred
...
int delete_file(int conn_sock, char* dir_name) { //char dir_name[] = "Downloadable files";
	struct stat st;
	int bytes_sent;
	char* file_path = NULL;
	char* file_name = NULL;
	...
	file_name = extract_file_name(file_path);
	if(file_name == NULL) {
		printf("Error:  something went wrong while extracting file name.\n");
		return -1;
	}
