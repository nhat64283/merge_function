...
void signInAccount(Account* root, int conn_sock, FILE* db) {
	char username[255];
	char password[64];
	int bytes_sent, bytes_received;
	int status;
...
void signOutAccount(Account* root, int conn_sock) { //not done ỷet 
	char username[255];
	int bytes_sent, bytes_received;
	Account* tmp;
	...
	while(tmp != NULL) {
		if(strcmp(tmp->username, username) == 0) {
			printf("Goodbye %s\n", username);
			//tmp->status = ACTIVE;  // no need to unblock here cuz we create multi process
			bytes_sent = send(conn_sock, MSG_TRUE, strlen(MSG_TRUE), 0);
			if (bytes_sent <= 0) {
				fprintf(stderr, "Failed to send signal to client. Try again.\n");
				return;
...
void fileTransfer(int conn_sock) {
	...
	do{
		...
		switch(choice) {
			case 1:
				status = recv_file(conn_sock, "transmission/files"); //loop receives files from client
				break;
			case 2:
				status = send_file(conn_sock, "Downloadable_Files"); //loop send files to client
				break;
			case 3:
				status = delete_file(conn_sock, "Downloadable_Files");
				break;
			default:
				break;
		}
